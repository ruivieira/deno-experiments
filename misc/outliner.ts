...
import { Task, Todo, Status } from "../common/tasks.ts";
...
function taskToString(task: Task): string {
  switch (task.status) {
    case Status.Done:
      return `${Colors.green("DONE")}\t[${task.priority}]\t${task.body}`;
    case Status.Todo:
      return `${Colors.brightRed("TODO")}\t[${task.priority}]\t${task.body}`;
    case Status.Later:
      return `${Colors.red("LATER")}\t[${task.priority}]\t${task.body}`;
    default:
