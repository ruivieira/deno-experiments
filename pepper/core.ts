...
export function createMarkdownTOC(content: string) {
    return content.split("\n")
        ...
        .map(x => {
            let parts = x.split("# ")
            return parts[0] + '# ' + `<a href="%${normaliseLink(parts[1] == undefined ? "" : parts[1])}">${parts[1]}</a>`
...
function globFiles(path: string, extension: string): Array<WalkEntry> {
    const files = [];
