...
const mod = function() {
    ...
    const Remarkable1 = Remarkable;
    return {
        Remarkable: Remarkable,
        utils: utils
    };
}();
var require$$0 = getDefaultExportFromNamespaceIfNotNamed(mod);
const { utils: { escapeHtml  }  } = require$$0;
function wikilinkRule(state, silent) {
    ...
    if (src.charCodeAt(start + 1) !== 91) return false;
    const labelStart = start + 2;
    let labelEnd = start + 2;
    state.pos = start + 2;
    let found = false;
    while(state.pos + 1 < posMax){
        if (src.charCodeAt(state.pos) === 93) {
            if (src.charCodeAt(state.pos + 1) === 93) {
                labelEnd = state.pos;
                found = true;
                break;
            }
...
const wikilink_open = function(tokens, idx, options) {
    return `<a href="${escapeHtml(encodeURIComponent(tokens[idx].href))}" class="wikilink">`;
};
const wikilink_close = function() {
    return "</a>";
};
var remarkableWikilink = function wikilink(md, opts) {
...
export { remarkableWikilink as default };
