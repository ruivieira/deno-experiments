...
class AbstractMatrix1 {
    static from1DArray(newRows, newColumns, newData) {
        let length = newRows * newColumns;
        if (length !== newData.length) {
            throw new RangeError("data length does not match given dimensions");
        }
        let newMatrix = new Matrix1(newRows, newColumns);
    ...
    static rowVector(newData) {
        let vector = new Matrix1(1, newData.length);
    ...
    static columnVector(newData) {
        let vector = new Matrix1(newData.length, 1);
        for(let i = 0; i < newData.length; i++){
    ...
    static zeros(rows, columns) {
        return new Matrix1(rows, columns);
    }
    static ones(rows, columns) {
        return new Matrix1(rows, columns).fill(1);
    }
    static rand(rows, columns, options = {
        ...
        let matrix = new Matrix1(rows, columns);
        for(let i = 0; i < rows; i++){
    ...
    static randInt(rows, columns, options = {
        ...
        if (!Number.isInteger(min1)) throw new TypeError("min must be an integer");
        if (!Number.isInteger(max1)) throw new TypeError("max must be an integer");
        if (min1 >= max1) throw new RangeError("min must be smaller than max");
    ...
    static min(matrix1, matrix2) {
        ...
        let result = new Matrix1(rows, columns);
        for(let i = 0; i < rows; i++){
            for(let j = 0; j < columns; j++){
    ...
    static checkMatrix(value) {
        return AbstractMatrix1.isMatrix(value) ? value : new Matrix1(value);
    }
    ...
    repeat(options = {
        ...
        let matrix = new Matrix1(this.rows * rows, this.columns * columns);
        for(let i = 0; i < rows; i++){
            for(let j = 0; j < columns; j++){
    ...
    getRowVector(index) {
        return Matrix1.rowVector(this.getRow(index));
    }
    setRow(index, array) {
        checkRowIndex(this, index);
    ...
    getColumnVector(index) {
        return Matrix1.columnVector(this.getColumn(index));
    }
    ...
    dot(vector2) {
        if (AbstractMatrix1.isMatrix(vector2)) vector2 = vector2.to1DArray();
    ...
    mmul(other) {
        other = Matrix1.checkMatrix(other);
        let m = this.rows;
        let n = this.columns;
        let p = other.columns;
        let result = new Matrix1(m, p);
    ...
    mmulStrassen(y) {
        ...
        function embed(mat, rows, cols) {
            ...
            if (r3 === rows && c3 === cols) {
                ...
                let resultat = AbstractMatrix1.zeros(rows, cols);
        ...
        function blockMult(a, b, rows, cols) {
            ...
            let m1 = blockMult(AbstractMatrix1.add(a11, a22), AbstractMatrix1.add(b11, b22), halfRows, halfCols);
            let m2 = blockMult(AbstractMatrix1.add(a21, a22), b11, halfRows, halfCols);
            let m3 = blockMult(a11, AbstractMatrix1.sub(b12, b22), halfRows, halfCols);
            let m4 = blockMult(a22, AbstractMatrix1.sub(b21, b11), halfRows, halfCols);
            let m5 = blockMult(AbstractMatrix1.add(a11, a12), b22, halfRows, halfCols);
            let m6 = blockMult(AbstractMatrix1.sub(a21, a11), AbstractMatrix1.add(b11, b12), halfRows, halfCols);
            let m7 = blockMult(AbstractMatrix1.sub(a12, a22), AbstractMatrix1.add(b21, b22), halfRows, halfCols);
            let c11 = AbstractMatrix1.add(m1, m4);
            c11.sub(m5);
            c11.add(m7);
            let c12 = AbstractMatrix1.add(m3, m5);
            let c21 = AbstractMatrix1.add(m2, m4);
            let c22 = AbstractMatrix1.sub(m1, m2);
            c22.add(m3);
            c22.add(m6);
            let resultat = AbstractMatrix1.zeros(2 * c11.rows, 2 * c11.columns);
    ...
    scaleRows(options = {
        ...
        if (!Number.isFinite(min1)) throw new TypeError("min must be a number");
        if (!Number.isFinite(max1)) throw new TypeError("max must be a number");
        if (min1 >= max1) throw new RangeError("min must be smaller than max");
        let newMatrix = new Matrix1(this.rows, this.columns);
        for(let i = 0; i < this.rows; i++){
    ...
    kroneckerProduct(other) {
        ...
        let p = other.rows;
        let q = other.columns;
        let result = new Matrix1(m * p, n * q);
        for(let i = 0; i < m; i++){
            for(let j = 0; j < n; j++){
                for(let k = 0; k < p; k++){
                    for(let l = 0; l < q; l++){
    ...
    kroneckerSum(other) {
        ...
        let m = this.rows;
        let n = other.rows;
        let AxI = this.kroneckerProduct(Matrix1.eye(n, n));
        let IxB = Matrix1.eye(m, m).kroneckerProduct(other);
        return AxI.add(IxB);
    }
    transpose() {
        let result = new Matrix1(this.columns, this.rows);
        for(let i = 0; i < this.rows; i++){
            for(let j = 0; j < this.columns; j++){
    ...
    subMatrix(startRow, endRow, startColumn, endColumn) {
        ...
        let newMatrix = new Matrix1(endRow - startRow + 1, endColumn - startColumn + 1);
    ...
    subMatrixRow(indices, startColumn, endColumn) {
        ...
        let newMatrix = new Matrix1(indices.length, endColumn - startColumn + 1);
    ...
    subMatrixColumn(indices, startRow, endRow) {
        ...
        let newMatrix = new Matrix1(endRow - startRow + 1, indices.length);
        for(let i = 0; i < indices.length; i++){
    ...
    setSubMatrix(matrix, startRow, startColumn) {
        matrix = Matrix1.checkMatrix(matrix);
        if (matrix.isEmpty()) {
            return this;
        }
    ...
    selection(rowIndices, columnIndices) {
        ...
        let newMatrix = new Matrix1(rowIndices.length, columnIndices.length);
...
AbstractMatrix1.prototype.klass = "Matrix";
if (typeof Symbol !== "undefined") {
    AbstractMatrix1.prototype[Symbol.for("nodejs.util.inspect.custom")] = inspectMatrix;
}
function compareNumbers(a, b) {
    return a - b;
}
AbstractMatrix1.random = AbstractMatrix1.rand;
AbstractMatrix1.randomInt = AbstractMatrix1.randInt;
AbstractMatrix1.diagonal = AbstractMatrix1.diag;
AbstractMatrix1.prototype.diagonal = AbstractMatrix1.prototype.diag;
AbstractMatrix1.identity = AbstractMatrix1.eye;
AbstractMatrix1.prototype.negate = AbstractMatrix1.prototype.neg;
AbstractMatrix1.prototype.tensorProduct = AbstractMatrix1.prototype.kroneckerProduct;
class Matrix1 extends AbstractMatrix1 {
    constructor(nRows, nColumns){
        super();
        if (Matrix1.isMatrix(nRows)) {
            return nRows.clone();
...
installMathOperations(AbstractMatrix1, Matrix1);
class BaseView extends AbstractMatrix1 {
...
class MatrixColumnView1 extends BaseView {
...
class MatrixColumnSelectionView1 extends BaseView {
...
class MatrixFlipColumnView1 extends BaseView {
    constructor(matrix3){
...
class MatrixFlipRowView1 extends BaseView {
    constructor(matrix4){
...
class MatrixRowView1 extends BaseView {
...
class MatrixRowSelectionView1 extends BaseView {
...
class MatrixSelectionView1 extends BaseView {
...
class MatrixSubView1 extends BaseView {
...
class MatrixTransposeView1 extends BaseView {
    constructor(matrix9){
...
class WrapperMatrix1D1 extends AbstractMatrix1 {
...
class WrapperMatrix2D1 extends AbstractMatrix1 {
    constructor(data1){
        super();
        this.data = data1;
...
function wrap1(array, options1) {
    if (Array.isArray(array)) {
        if (array[0] && Array.isArray(array[0])) {
            return new WrapperMatrix2D1(array);
        } else {
            return new WrapperMatrix1D1(array, options1);
        }
    } else {
        throw new Error("the argument is not an array");
    }
}
class LuDecomposition1 {
    constructor(matrix10){
        matrix10 = WrapperMatrix2D1.checkMatrix(matrix10);
    ...
    solve(value) {
        value = Matrix1.checkMatrix(value);
        let lu1 = this.LU;
        let rows3 = lu1.rows;
        if (rows3 !== value.rows) {
    ...
    get lowerTriangularMatrix() {
        ...
        let X = new Matrix1(rows3, columns2);
...
class QrDecomposition1 {
    constructor(value1){
        value1 = WrapperMatrix2D1.checkMatrix(value1);
        let qr = value1.clone();
        let m = value1.rows;
...
class SingularValueDecomposition1 {
    constructor(value2, options1 = {
        ...
        value2 = WrapperMatrix2D1.checkMatrix(value2);
        if (value2.isEmpty()) {
        ...
        let U = new Matrix1(m1, nu);
        let V = new Matrix1(n1, n1);
    ...
    solve(value) {
        ...
        let Ls = Matrix1.zeros(scols, scols);
        ...
        let vrows = V1.rows;
        let urows = U1.rows;
        let VLU = Matrix1.zeros(vrows, urows);
    ...
    solveForDiagonal(value) {
        return this.solve(Matrix1.diag(value));
    }
    inverse() {
        let V1 = this.V;
        let e1 = this.threshold;
        ...
        let X = new Matrix1(vrows, this.s.length);
    ...
    get diagonalMatrix() {
        return Matrix1.diag(this.s);
    }
}
function inverse1(matrix11, useSVD = false) {
    matrix11 = WrapperMatrix2D1.checkMatrix(matrix11);
    if (useSVD) {
        return new SingularValueDecomposition1(matrix11).inverse();
    } else {
        return solve1(matrix11, Matrix1.eye(matrix11.rows));
    }
}
function solve1(leftHandSide, rightHandSide, useSVD = false) {
    leftHandSide = WrapperMatrix2D1.checkMatrix(leftHandSide);
    rightHandSide = WrapperMatrix2D1.checkMatrix(rightHandSide);
    if (useSVD) {
        return new SingularValueDecomposition1(leftHandSide).solve(rightHandSide);
    } else {
        return leftHandSide.isSquare() ? new LuDecomposition1(leftHandSide).solve(rightHandSide) : new QrDecomposition1(leftHandSide).solve(rightHandSide);
    }
}
function determinant1(matrix11) {
    matrix11 = Matrix1.checkMatrix(matrix11);
    if (matrix11.isSquare()) {
        if (matrix11.columns === 0) {
            return 1;
        }
        let a1, b, c, d;
        if (matrix11.columns === 2) {
            a1 = matrix11.get(0, 0);
            b = matrix11.get(0, 1);
            c = matrix11.get(1, 0);
            d = matrix11.get(1, 1);
            return a1 * d - b * c;
        } else if (matrix11.columns === 3) {
            let subMatrix0, subMatrix1, subMatrix2;
            subMatrix0 = new MatrixSelectionView1(matrix11, [
                1,
                2
            ], [
                1,
            ...
            subMatrix1 = new MatrixSelectionView1(matrix11, [
                1,
                2
            ], [
                0,
            ...
            subMatrix2 = new MatrixSelectionView1(matrix11, [
                1,
                2
            ], [
                0,
            ...
            b = matrix11.get(0, 1);
            c = matrix11.get(0, 2);
            return a1 * determinant1(subMatrix0) - b * determinant1(subMatrix1) + c * determinant1(subMatrix2);
        } else {
            return new LuDecomposition1(matrix11).determinant;
        }
    } else {
...
function dependenciesOneRow(error, matrix11, index, thresholdValue = 0.000000001, thresholdError = 0.000000001) {
    if (error > thresholdError) {
        return new Array(matrix11.rows + 1).fill(0);
    } else {
        let returnArray = matrix11.addRow(index, [
            0
        ]);
...
function linearDependencies1(matrix11, options2 = {
}) {
    const { thresholdValue =0.000000001 , thresholdError =0.000000001  } = options2;
    matrix11 = Matrix1.checkMatrix(matrix11);
    let n2 = matrix11.rows;
    let results = new Matrix1(n2, n2);
    for(let i3 = 0; i3 < n2; i3++){
        let b = Matrix1.columnVector(matrix11.getRow(i3));
        let Abis = matrix11.subMatrixRow(xrange(n2, i3)).transpose();
        let svd = new SingularValueDecomposition1(Abis);
        let x = svd.solve(b);
        let error = Matrix1.sub(b, Abis.mmul(x)).abs().max();
        results.setRow(i3, dependenciesOneRow(error, x, i3, thresholdValue, thresholdError));
    }
    return results;
}
function pseudoInverse1(matrix11, threshold = Number.EPSILON) {
    matrix11 = Matrix1.checkMatrix(matrix11);
    if (matrix11.isEmpty()) {
        return matrix11.transpose();
    }
    let svdSolution = new SingularValueDecomposition1(matrix11, {
        autoTranspose: true
    });
    ...
    return V1.mmul(Matrix1.diag(s3).mmul(U1.transpose()));
}
function covariance1(xMatrix, yMatrix = xMatrix, options2 = {
}) {
    xMatrix = new Matrix1(xMatrix);
    let yIsSame = false;
    if (typeof yMatrix === "object" && !Matrix1.isMatrix(yMatrix) && !Array.isArray(yMatrix)) {
        options2 = yMatrix;
        yMatrix = xMatrix;
        yIsSame = true;
    } else {
        yMatrix = new Matrix1(yMatrix);
    }
    if (xMatrix.rows !== yMatrix.rows) {
        throw new TypeError("Both matrices must have the same number of rows");
    }
    const { center =true  } = options2;
    if (center) {
...
function correlation1(xMatrix, yMatrix = xMatrix, options2 = {
}) {
    xMatrix = new Matrix1(xMatrix);
    let yIsSame = false;
    if (typeof yMatrix === "object" && !Matrix1.isMatrix(yMatrix) && !Array.isArray(yMatrix)) {
    ...
    const { center =true , scale =true  } = options2;
    if (center) {
    ...
    for(let i3 = 0; i3 < corr.rows; i3++){
        for(let j2 = 0; j2 < corr.columns; j2++){
            corr.set(i3, j2, corr.get(i3, j2) * (1 / (sdx[i3] * sdy[j2])) * (1 / (xMatrix.rows - 1)));
        }
    }
    return corr;
}
class EigenvalueDecomposition1 {
...
class CholeskyDecomposition1 {
    constructor(value4){
        value4 = WrapperMatrix2D1.checkMatrix(value4);
        if (!value4.isSymmetric()) {
            throw new Error("Matrix is not symmetric");
        }
        let a1 = value4;
        let dimension = a1.rows;
        let l = new Matrix1(dimension, dimension);
    ...
    solve(value) {
        value = WrapperMatrix2D1.checkMatrix(value);
        let l1 = this.L;
...
export { AbstractMatrix1 as AbstractMatrix, CholeskyDecomposition1 as CHO, CholeskyDecomposition1 as CholeskyDecomposition, EigenvalueDecomposition1 as EVD, EigenvalueDecomposition1 as EigenvalueDecomposition, LuDecomposition1 as LU, LuDecomposition1 as LuDecomposition, Matrix1 as Matrix, MatrixColumnSelectionView1 as MatrixColumnSelectionView, MatrixColumnView1 as MatrixColumnView, MatrixFlipColumnView1 as MatrixFlipColumnView, MatrixFlipRowView1 as MatrixFlipRowView, MatrixRowSelectionView1 as MatrixRowSelectionView, MatrixRowView1 as MatrixRowView, MatrixSelectionView1 as MatrixSelectionView, MatrixSubView1 as MatrixSubView, MatrixTransposeView1 as MatrixTransposeView, nipals as NIPALS, nipals as Nipals, QrDecomposition1 as QR, QrDecomposition1 as QrDecomposition, SingularValueDecomposition1 as SVD, SingularValueDecomposition1 as SingularValueDecomposition, WrapperMatrix1D1 as WrapperMatrix1D, WrapperMatrix2D1 as WrapperMatrix2D, correlation1 as correlation, covariance1 as covariance, determinant1 as determinant, inverse1 as inverse, linearDependencies1 as linearDependencies, pseudoInverse1 as pseudoInverse, solve1 as solve, wrap1 as wrap };
export { Matrix1 as default };
